...
class Solution:
        ...
        with torch.no_grad():
            for module in model.children():
                ...
                if isinstance(module, nn.Linear):
                    ...
                    if x.dim() >= 2: #dims usually follow (batch_in, #neurons)
    ...
    def compute_gradient_stats(self, model: nn.Module, x: torch.Tensor, y: torch.Tensor) -> List[Dict[str, float]]:
        ...
        #need to zero grads for each iteration
        model.zero_grad()
        output = model(x)
        loss = nn.MSELoss()(output,y) #having the MSELoss()(output,y) basically calls forward pass and MSE
        loss.backward()
        stats = []
